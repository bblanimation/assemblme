...
def make_cylinder(radius:float, height:float, vertices:int, co:Vector=Vector((0,0,0)), bot_face:bool=True, top_face:bool=True, flip_normals:bool=False, smooth:bool=True, seams:bool=True, bme:bmesh=None):
    ...
    # create edges (only necessary so we can set smoothness)
    for i in range(len(top_verts)):
        v1 = top_verts[i]
        v2 = top_verts[(i-1)]
        v3 = bot_verts[i]
        v4 = bot_verts[(i-1)]
        bme.edges.new((v1, v2)).smooth = False
        bme.edges.new((v3, v4)).smooth = False
    ...
    if top_face:
        the_top_face = bme.faces.new(top_verts if not flip_normals else top_verts[::-1])
        created_faces.append(the_top_face)
    if bot_face:
        the_bot_face = bme.faces.new(bot_verts[::-1] if not flip_normals else bot_verts)
        created_faces.append(the_bot_face)
